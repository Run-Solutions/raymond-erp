...
import { Accesorio, accesoriosApi } from '@/services/taller-r1/accesorios.service';
import { ubicacionesApi, Ubicacion } from '@/services/taller-r1/ubicaciones.service';
import { Truck, AlertCircle, Check, MapPin, Archive } from 'lucide-react';
import { VisuallyHidden } from '@radix-ui/react-visually-hidden';
import { DialogTitle } from '@radix-ui/react-dialog';

interface AccesorioMovilizacionModalProps {
    open: boolean;
...
export function AccesorioMovilizacionModal({ open, onOpenChange, accesorio, onSuccess }: AccesorioMovilizacionModalProps) {
    ...
    const handleUbicacionChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        try {
            ...
            // We do NOT filter out occupied sublocations because the user wants to place it anywhere without strict constraints
            setSubUbicaciones(res);
        } catch (error) {
    ...
    const handleMovilizar = async () => {
        if (!accesorio || !selectedUbicacion || !selectedSubUbicacion) {
        ...
        try {
            ...
            // We update the accessory with the new location and sub-location.
            // This is just a logical assignment for accessories and does not mark the sub-location as physically occupied.
            await accesoriosApi.update(accesorio.id_accesorio, {
                ubicacion: selectedUbicacion,
                sub_ubicacion: selectedSubUbicacion
            });

            toast.success('Accesorio movilizado con éxito');
            onSuccess();
            onOpenChange(false);
        } catch (error: any) {
            console.error(error);
            toast.error(error.response?.data?.message || 'Error al movilizar el accesorio');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <Dialog open={open} onOpenChange={(openVal) => {
            ...
            <DialogContent className="max-w-xl p-0 overflow-hidden bg-white rounded-3xl shadow-2xl border-0 h-auto max-h-[90vh] flex flex-col">
                <VisuallyHidden>
                    <DialogTitle>Movilizar Accesorio</DialogTitle>
                </VisuallyHidden>
                    ...
                    <>
                        <div className="shrink-0 bg-gradient-to-br from-red-600 to-red-800 px-8 py-8 relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    <h2 className="text-2xl font-black text-white tracking-tight">Movilizar Accesorio</h2>
                                    <p className="text-red-100 font-medium text-sm mt-1">Registrar cambio de ubicación sin ocupar la posición</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-gray-50/50">
                            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm relative overflow-hidden">
                                ...
                                <div>
                                    <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Tipo / Modelo</p>
                                    <p className="text-sm font-black text-gray-900 mt-1 truncate">{accesorio.tipo} - {accesorio.modelo}</p>
                                    <p className="font-black text-slate-800 text-lg tracking-tight">Serial: {accesorio.serial}</p>
                                </div>
                                <div className="col-span-2 pt-2 border-t border-gray-50 mt-4">
                                    <p className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">Ubicación Actual</p>
                                    <div className="flex items-center gap-3 mt-1">
                                        <p className="text-sm font-black text-gray-900 truncate">{accesorio.rel_ubicacion?.nombre_ubicacion || accesorio.ubicacion || 'N/A'}</p>
                                        <p className="text-[10px] font-black text-red-600 bg-red-50 px-2 py-0.5 rounded border border-red-100">
                                            Posición {accesorio.rel_sub_ubicacion?.nombre || accesorio.sub_ubicacion || accesorio.rack || 'N/A'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-5">
                                    <div>
                                        ...
                                        >
                                            ...
                                            {ubicaciones
                                                .filter(u => u.Clase === 'Accesorio' || u.Clase === 'Accesorios')
                                                .map((u) => (
                                    ...
                                    <div>
                                        <label className="block text-xs font-black text-gray-700 uppercase tracking-wider mb-2">Posición (Sub-Ubicación)</label>
                                        <select
