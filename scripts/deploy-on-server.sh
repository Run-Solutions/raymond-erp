...
docker compose -f docker compose.prod.images.yml run --rm api npx prisma@5.19.1 migrate deploy || {
    echo -e "${YELLOW}   ⚠️  Migraciones fallaron, revisar manualmente${NC}"
    echo "   Para revisar: docker compose -f docker compose.prod.images.yml run --rm api npx prisma@5.19.1 migrate status"
}
echo ""
